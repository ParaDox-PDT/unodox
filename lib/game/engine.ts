...
export type Player = { id: string; name: string; hand: UnoCard[] };
...
export function createGame(playerNames = ["You", "Lena", "Milo"]): GameState {
  ...
  const players = playerNames.map((name, i) => ({ id: `player-${i}`, name, hand: deck.splice(-7) }));
  return { players, drawPile: deck, discardPile: [opening], currentPlayerIndex: 0, direction: 1, activeColor: opening.color, pendingDrawCount: 0, drawStackType: null, gameStatus: "playing", winnerId: null, awaitingColorFor: null, hasDrawnCard: false, selectedComboCards: [] };
}
...
export function canPlayCards(cards: UnoCard[], state: GameState, playerId: string) {
  if (!cards.length || !cards.every(numeric) || state.pendingDrawCount) return false;
  const player = state.players[state.currentPlayerIndex];
...
export function playCards(state: GameState, playerId: string, cardIds: string[]): GameState {
  ...
  player.hand = player.hand.filter(card => !cardIds.includes(card.id)); next.discardPile.push(...cards); next.activeColor = cards.at(-1)!.color!; next.selectedComboCards = [];
...
export function playCard(state: GameState, playerId: string, cardId: string): GameState {
  ...
  if (finishIfEmpty(next, player)) return next;
...
export function selectWildColor(state: GameState, playerId: string, color: CardColor): GameState {
  const next = clone(state); if (next.gameStatus !== "playing" || next.awaitingColorFor !== playerId || !COLORS.includes(color)) throw new Error("Invalid wild color selection.");
...
export function drawCard(state: GameState, playerId: string): GameState {
  ...
  const drawn = take(next, 1); player.hand.push(...drawn); next.hasDrawnCard = true;
...
export function acceptPendingDraw(state: GameState, playerId: string): GameState {
  ...
  next.players[next.currentPlayerIndex].hand.push(...take(next, next.pendingDrawCount)); next.pendingDrawCount = 0; next.drawStackType = null; advance(next); return next;
}
